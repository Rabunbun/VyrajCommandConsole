...
async function main() {
  ...
  try {
    ...
    const slotTopology = await readSlotTopology(files.typeDogma, publishedShips);
    const refreshedAt = new Date();
    const hulls = Array.from(publishedShips.values())
      .map((ship) => ({
        ...ship,
        ...slotTopology.get(ship.typeId),
        highSlots: slotTopology.get(ship.typeId)?.highSlots ?? 0,
        lowSlots: slotTopology.get(ship.typeId)?.lowSlots ?? 0,
        midSlots: slotTopology.get(ship.typeId)?.midSlots ?? 0,
        rigSlots: slotTopology.get(ship.typeId)?.rigSlots ?? 0,
    ...
    console.log("Slot topology verification from SDE Dogma attributes:");

    for (const hull of verified) {
      console.log(
        `- ${hull.typeName}: high ${hull.highSlots}, mid ${hull.midSlots}, low ${hull.lowSlots}, rig ${hull.rigSlots}`
      );
    }
...
async function readSlotTopology(
  filePath: string,
  ships: Map<number, unknown>
) {
  const shipTypeIds = new Set(ships.keys());
  const topology = new Map<number, {
    highSlots: number;
    lowSlots: number;
    midSlots: number;
    rigSlots: number;
